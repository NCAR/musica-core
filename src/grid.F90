...
module musica_grid
  ...
  type grid_section_t
    ...
    real(kind=musica_dk) :: mid_point_
  ...
  contains
    ...
    procedure :: mid_point   => grid_section_mid_point
  ...
  contains
    ...
    procedure :: set            => grid_iterator_set
  ...
  contains
    ...
    procedure :: private_constructor_midpoints
  end type grid_t

  interface grid_t
    module procedure :: constructor_bounds
    module procedure :: constructor_midpoints
  end interface
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !> Constructor of grid_t objects from a set of mid-points
  !!
  !! The grid is assumed to be continous with section boundaries spaced mid-
  !! way between mid-points. The overall lower and upper boundaries are
  !! set such that the adjacent mid-point is mid-way between its sections
  !! lower and upper boundaries.
  function constructor_midpoints( midpoints, units ) result( new_grid )

    !> Grid
    type(grid_t)                     :: new_grid
    !> Mid-points for each section
    real(kind=musica_dk), intent(in) :: midpoints(:)
    !> Base grid units
    type(string_t),       intent(in) :: units

    call new_grid%private_constructor_midpoints( midpoints, units )

  end function constructor_midpoints
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine private_constructor_bounds( this, lower_bounds, upper_bounds,    &
    ...
    this%sections_(:)%mid_point_   =                                          &
      ( upper_bounds(:) + lower_bounds(:) ) / 2.0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !> Constructor of grid_t objects from mid-points
  !!
  !! This should only be called by constructors of extending types
  subroutine private_constructor_midpoints( this, midpoints, units )
    ...
    allocate( this%sections_( size( midpoints ) ) )
    this%sections_(:)%mid_point_ = midpoints(:)
    do i_sect = 1, size( midpoints )
      if( i_sect .gt. 1 ) then
        this%sections_( i_sect )%lower_bound_ =                               &
          ( this%sections_( i_sect )%mid_point_ +                             &
            this%sections_( i_sect - 1 )%mid_point_ ) / 2.0_musica_dk
      end if
      if( i_sect .lt. size( midpoints ) ) then
        this%sections_( i_sect )%upper_bound_ =                               &
          ( this%sections_( i_sect )%mid_point_ +                             &
            this%sections_( i_sect + 1 )%mid_point_ ) / 2.0_musica_dk
      end if
    end do
    this%sections_( 1 )%lower_bound_ =                                        &
      this%sections_( 1 )%mid_point_ -                                        &
        ( this%sections_( 1 )%upper_bound_ -                                  &
          this%sections_( 1 )%mid_point_ )
    this%sections_( size( midpoints ) )%upper_bound_ =                        &
      this%sections_( size( midpoints ) )%mid_point_ +                        &
        ( this%sections_( size( midpoints ) )%mid_point_ -                    &
          this%sections_( size( midpoints ) )%lower_bound_ )
    this%units_ = units

  end subroutine private_constructor_midpoints
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !> Returns the mid-point for a particular grid section
  real(kind=musica_dk) elemental function mid_point( this, iterator )
    ...
    mid_point = this%sections_( iterator%current_section_ )%mid_point( )
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  real(kind=musica_dk) elemental function grid_section_overlap( a, b )        &
    ...
    overlap = 0.0_musica_dk
    if( a%lower_bound_ .le. b%upper_bound_ .and.                              &
        b%lower_bound_ .le. a%upper_bound_ ) then
      overlap = max( 0.0_musica_dk, min( a%upper_bound_, b%upper_bound_ ) -   &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !> Returns the mid-point of the grid section in the base units of the grid
  real(kind=musica_dk) elemental function grid_section_mid_point( this )

    class(grid_section_t), intent(in) :: this

    grid_section_mid_point = this%mid_point_

  end function grid_section_mid_point
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !> Sets the iterator to a specific position
  subroutine grid_iterator_set( this, pos )
    ...
    call assert_msg( 853492316, pos .gt. 0 .and. pos .le. this%last_section_, &
                     "Invalid position specified for grid iterator" )
    this%current_section_ = pos

  end subroutine grid_iterator_set
